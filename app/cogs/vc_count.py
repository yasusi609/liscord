...
class vc_count(commands.Cog):
    ...
    async def voice_update(self, member:discord.Member, before:discord.VoiceState, after:discord.VoiceState):
        ...
        if (before.channel is None or check):
            ...
            embed=None
            if i==1:
                embed=cogs.bin.activity.callemb(after,member)
        ...
        if before.self_video is False and after.self_video is True:
            await client.send(f"@everyone <@{member.id}> が、{after.channel.name}でカメラ配信を始めました。",embed=cogs.bin.activity.stream(after,member,title="カメラ配信"))
            ...
            mst,embed = cogs.bin.activity.activity(after,member)
